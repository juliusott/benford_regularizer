...
import os
import argparse
from benford_regularizer import quantile_loss, compute_kl, layerwise_quantile_loss, compute_kl_modulewise
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    print("dist url main worker", args.dist_url)
    if args.distributed:
        ...
        if args.multiprocessing_distributed:
            ...
            print("starting on rank" , args.rank)
        dist.init_process_group(backend=args.dist_backend, init_method=args.dist_url,
                                world_size=args.world_size, rank=args.rank)
    # create model
    if args.pretrained:
        print("=> using pre-trained model '{}'".format(args.model))
        model = models.__dict__[args.model](weights="IMAGENET1K_V2")
    else:
    ...
    if args.resume and not args.pretrained:
        if os.path.isfile(args.resume):
            ...
            if args.gpu is None and not torch.cuda.is_available():
    ...
    train_dataset.set_transform(transform=transform_train_fn)
    ...
    checkpoint_path = f'{save_dir}/ckpt_{args.model}_{args.seed}{args.benford}.pth'

    if not os.path.isdir(log_dir) and not os.path.isdir(save_dir) and (args.rank % ngpus_per_node == 0):
        os.makedirs(log_dir)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (args.rank % ngpus_per_node == 0):
            logger.add_scalar("val_acc", acc1, epoch)
...
def test(loader, model, device, args):
    batch_time = AverageMeter('Time', ':6.3f')
    data_time = AverageMeter('Data', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    ...
    top_5_predictions = np.zeros((100000, 5))

    end = time.time()
    for i, (batch, index) in enumerate(loader):
        images = batch["image"]
        # measure data loading time
        data_time.update(time.time() - end)
        ...
        top_5_predictions[index] = pred.squeeze().cpu().numpy().astype(np.int16)
...
def validate(loader, model, criterion, args):
    ...
    if args.distributed and (len(loader.sampler) * args.world_size < len(loader.dataset)):
        ...
        run_validate(aux_val_loader, len(loader))
