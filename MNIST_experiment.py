...
def visualize():
    ...
    test_loss = np.concatenate(
        [np.expand_dims(np.load(f"./experiments/mnist/test_loss_{seed}.npy"), axis=1) for seed in seeds], axis=1)
    print(test_loss.shape)
    test_acc = np.concatenate(
        [np.expand_dims(np.load(f"./experiments/mnist/test_acc_{seed}.npy"), axis=1) for seed in seeds], axis=1)
